...
addEventListener("keydown", (e) => {
    if (currentRow <= 5 && win == false) {
        if (/^[a-zA-Z]$/.test(e.key) && currentBox <= 4) {
            e.preventDefault()
            for (let i = 0; i < 6; i++) {
                if (row[i].id == (currentRow + 1).toString()) {
                    for (let j = 0; j < 5; j++) {
                        ...
                        if (cubeInRow[j].id == (currentBox + 1).toString()) {
                            cubeInRow[j].innerHTML = e.key.toUpperCase()

                        }
                    }
                }
            }
            wordsAdded[currentRow].push(e.key.toUpperCase())
            currentBox++
        }
        ...
        if (e.key == 'Enter' && currentBox == 5) {
            if (words.includes(wordsAdded[currentRow].join('').toLowerCase())) {
                ...
                wordsAdded[currentRow].forEach((ele, num) => {
                    ...
                    if (genWrdArr.includes(ele) && genLetterCount[ele] > 0 && cubes[num].style.backgroundColor != 'rgb(88, 163, 81)') {
